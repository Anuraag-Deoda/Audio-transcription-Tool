...
const AudioTranscriptionApp = () => {
  ...
  const calculateCustomBlockTiming = (blockText) => {
    ...
    // Find matching words in the transcription
    for (let i = 0; i < words.length; i++) {
      const word = words[i];
      const wordText = (word.text || word.word || '').toLowerCase().replace(/[^\w]/g, '');
      
      // Check if this word matches any of our block words
      for (let j = 0; j < blockWords.length; j++) {
        const blockWord = blockWords[j].replace(/[^\w]/g, '');
        if (wordText === blockWord) {
          if (startTime === null) {
      ...
      // If we've matched all words, we can stop
      if (matchedWords >= blockWords.length) {
        break;
      }
    }
                            ...
                            <div className="d-flex align-items-center justify-content-between mb-2">
                              <div className="btn-group btn-group-sm">
                                <button
                                  ...
                                  onClick={zoomOut}
                                  disabled={zoomLevel <= 1} // Min zoom 0.5x
                                  title="Zoom Out"
                                >
                                  <ZoomOut size={14} />
                                </button>
                                <button
                                  className="btn btn-outline-secondary"
                                  onClick={zoomIn}
                                  disabled={zoomLevel >= 5} // Max zoom 5x
                                  title="Zoom In"
                                >
                                  <ZoomIn size={14} />
                                </button>
                            ...
                            <div className="card-body border-bottom bg-light p-3">
                              <div className="d-flex align-items-center gap-2">
                                ...
                                <input
                                  ...
                                  placeholder={`1-${editableBlocks.length}`}
                                ...
                                <button
                                  ...
                                  disabled={
                                    selectedBlockIndex === null || !quickReorderPosition
                                  }
                                  title="Move selected block to position"
                              ...
                              {selectedBlockIndex === null && (
                                ...
                                >
                                  Select a block to enable quick reorder
                                </small>
                              )}
                            </div>
                          )}
                          <div className="card-body p-0">
                            ...
                            >
                              {editableBlocks.map((block, index) => {
                                const words = getWordsForBlock(block, index);

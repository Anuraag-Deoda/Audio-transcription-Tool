...
const AudioTranscriptionApp = () => {
  ...
  const [originalTranscriptionData, setOriginalTranscriptionData] = useState(
    []
  ); // Stores the raw, unedited transcription data
  ...
  const blockListRef = useRef(); // ADD THIS
  ...
  // TextUtils is a dependency for the timing functions, so it's included here.
  const TextUtils = {
  ...
  /**
   * Calculates the start and end time for a given block of text by matching it against the detailed word-level transcription.
   * It also intelligently adjusts the end time to account for natural pauses after sentences or at the end of the audio.
   *
   * @param {string} blockText - The text content of the custom block to be timed.
   * @param {number} searchStartTime - The time in the audio from which to start searching for the text block.
   * @param {number} audioDuration - The total duration of the audio.
   * @returns {{start: number, end: number}} An object containing the calculated start and end times for the block.
   */
  const calculateCustomBlockTiming = useCallback(
    (blockText, searchStartTime = 0, audioDuration) => {
      console.log(
        `--- calculateCustomBlockTiming for: "${blockText.substring(
          0,
          30
        )}..." ---`
      );
      console.log(
        `  Initial searchStartTime: ${searchStartTime.toFixed(
          3
        )}, Audio Duration: ${audioDuration.toFixed(3)}`
      );

      if (!transcriptionData || !transcriptionData.words) {
        console.warn(
          "  transcriptionData or transcriptionData.words is missing. Returning default timing."
        );
      ...
      if (blockWords.length === 0) {
        console.log(
          "  Block text resulted in no words. Returning timing based on searchStartTime."
        );
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        const transcriptionWord = TextUtils.normalizeText(
          word.text || word.word || ""
        );
        ...
        if (transcriptionWord === blockWord) {
          ...
          if (currentWordIndexInBlock === blockWords.length) {
            console.log(
              `  ✓ Found complete sequence for "${blockText.substring(
                0,
                30
              )}..."`
            );
          ...
          console.log(
            `  ✗ Sequence broken at word: "${word.text}", expected: "${blockWord}"`
          );
      ...
      if (
        currentWordIndexInBlock < blockWords.length &&
        bestMatch &&
        bestMatch.matchedWords > 0
      ) {
        console.log(
          `  ⚠ Using best partial match: ${bestMatch.matchedWords} of ${
            blockWords.length
          } words for "${blockText.substring(0, 30)}..."`
        );
      ...
      if (endTime !== null && lastMatchedWordIndex !== -1) {
        const blockEndsWithPunctuation =
          TextUtils.endsWithPunctuation(blockText);
        console.log(
          `  Block ends with punctuation: ${blockEndsWithPunctuation}`
        );
        ...
        console.log(
          `  Last matched word index: ${lastMatchedWordIndex}, Next word index: ${nextWordIndex}, Total words: ${words.length}`
        );

        if (nextWordIndex >= words.length) {
          const pauseExtension = Math.min(
            0.75,
            (audioDuration - endTime - 0.1) * 0.9
          );
          const extendedTime = endTime + pauseExtension;
          finalEndTime = Math.min(extendedTime, audioDuration);
          console.log(
            `  Last block, extending from ${endTime.toFixed(
              3
            )} by 0.3s to ${extendedTime.toFixed(
              3
            )}, capped at audio duration: ${finalEndTime.toFixed(3)}`
          );
        } else {
          const nextWord = words[nextWordIndex];
          const timeBetweenWords = nextWord.start - endTime;
          console.log(
            `  Time between current block end (${endTime.toFixed(
              3
            )}) and next word start (${nextWord.start.toFixed(
              3
            )}): ${timeBetweenWords.toFixed(3)}`
          );

          if (timeBetweenWords > 0.1) {
            console.log(
              `time between words for "${blockText.substring(
                0,
                30
              )}...": ${timeBetweenWords.toFixed(3)}`
            );
            const pauseExtension = Math.min(1.1, timeBetweenWords * 0.9);
            if (timeBetweenWords > 1) {
              console.log(
                `  Significant gap found. Added pause time: ${pauseExtension.toFixed(
                  3
                )}, new end: ${finalEndTime.toFixed(3)}`
              );
              finalEndTime = endTime + pauseExtension;
            } else {
              finalEndTime = nextWord.start;
            }
            console.log(
              `  Significant gap found. Added pause time: ${pauseExtension.toFixed(
                3
            ...
            console.log(
              `  Gap (${timeBetweenWords.toFixed(
                3
              )}s) not significant enough (threshold 0.25s). No extension.`
            );
          }
        }
      } else {
        console.log(
          `  No valid endTime or lastMatchedWordIndex. No end time adjustment.`
        );
      }
      ...
      console.log(
        `--- FINAL RESULT for "${blockText.substring(
          0,
          30
        )}...": Start: ${result.start.toFixed(3)}, End: ${result.end.toFixed(
          3
        )}, Duration: ${(result.end - result.start).toFixed(3)} ---`
      );
      return result;
    },
    [transcriptionData]
  ); // Added transcriptionData as a dependency for useCallback
  ...
  const calculateCustomBlockTiming_custom = (
    blockText,
    searchStartTime = 0,
    audioDuration
  ) => {
    console.log(
      `--- calculateCustomBlockTiming_custom for: "${blockText.substring(
        0,
        30
      )}..." ---`
    );
    ...
    const words = transcriptionData.words;
    const blockWords = TextUtils.extractWords(blockText).map((word) =>
      TextUtils.normalizeText(word)
    );
    ...
    const areWordsSimilar = (word1, word2) => {
      ...
      if (
        normalizedWord1.startsWith(normalizedWord2) ||
      ...
      if (
        homophoneBank[normalizedWord1] &&
        homophoneBank[normalizedWord1].includes(normalizedWord2)
      ) {
        return true;
      }
      if (
        homophoneBank[normalizedWord2] &&
        homophoneBank[normalizedWord2].includes(normalizedWord1)
      ) {
        return true;
      }
    ...
    if (endTime !== null && lastMatchedWordIndex !== -1) {
      ...
      console.log(`  Block ends with punctuation: ${blockEndsWithPunctuation}`);
  ...
  const publishCustomBlocks = () => {
    ...
    const publishedBlocks = customBlocks.map((customBlock, index) => {
      // Use the corrected calculateCustomBlockTiming_custom function, passing 'duration'
      const timing = calculateCustomBlockTiming_custom(
        customBlock.text,
        currentSearchStartTime,
        duration
      );
  ...
  useEffect(() => {
    // If switching to custom view for the first time (or after a reset), initialize it.
    // This preserves user edits if they switch away and back to the custom view.
    if (
      transcriptionData &&
      viewMode === "custom" &&
      customText.trim() === ""
    ) {
      const baseViewMode = "sentences";
      const rawData = transcriptionData[baseViewMode] || [];
      const combinedText = rawData
        .map((item) => item.text || item.word)
        .join(" ");
      handleCustomTextChange(combinedText);
    }
  }, [transcriptionData, viewMode, customText]);
  ...
  useEffect(() => {
    if (transcriptionData && viewMode !== "custom" && !publishedBlocks) {
      ...
      const blocks = getRawDisplayData(transcriptionData).map((item, index) => {
        ...
        const adjustedTiming = calculateCustomBlockTiming(
          blockText,
          item.start,
          duration
        );
  ...
  }, [transcriptionData, viewMode, calculateCustomBlockTiming, duration]);
  ...
  useEffect(() => {
    if (selectedBlockIndex !== null) {
      ...
      if (block) {
        const elementId = `block-editor-item-${block.id}`;
  ...
  }, [selectedBlockIndex, editableBlocks]);
      ...
      reader.onload = (e) => {
        ...
        try {
          ...
          if (transcriptionData && transcriptionData.sentences) {
            const audioText = transcriptionData.sentences
              .map((s) => s.text)
              .join(" ");
            ...
            const cleanedAudioTextNoSpaces = cleanedAudioText.replace(
              /\s/g,
              ""
            );
            const cleanedExtractedTextNoSpaces = cleanedExtractedText.replace(
              /\s/g,
              ""
            );

            // Check if one is a substring of the other (after removing spaces)
            const isSubsequenceSimilar =
              cleanedAudioTextNoSpaces.includes(cleanedExtractedTextNoSpaces) ||
              cleanedExtractedTextNoSpaces.includes(cleanedAudioTextNoSpaces);

            const minLengthForRatio = Math.min(
              cleanedAudioTextNoSpaces.length,
              cleanedExtractedTextNoSpaces.length
            );
            const maxLengthForRatio = Math.max(
              cleanedAudioTextNoSpaces.length,
              cleanedExtractedTextNoSpaces.length
            );
            ...
            if (maxLengthForRatio > 0) {
              ...
              let matchCount = 0;
              let j = 0; // pointer for cleanedExtractedTextNoSpaces
              for (
                let i = 0;
                i < cleanedAudioTextNoSpaces.length &&
                j < cleanedExtractedTextNoSpaces.length;
                i++
              ) {
                if (
                  cleanedAudioTextNoSpaces[i] ===
                  cleanedExtractedTextNoSpaces[j]
                ) {
                  matchCount++;
                  j++;
                }
            ...
            const lengthDifferenceRatio =
              maxLengthForRatio > 0
                ? Math.abs(
                    cleanedAudioText.length - cleanedExtractedText.length
                  ) / maxLengthForRatio
                : 0;
            const areTextsEffectivelySimilar =
              isSubsequenceSimilar ||
              (characterSimilarity >= SIMILARITY_THRESHOLD &&
                lengthDifferenceRatio < 0.1);

            if (!areTextsEffectivelySimilar) {
              setError(
                "Warning: The text in the uploaded XHTML file appears to be significantly different from the audio transcription. Please ensure you have uploaded the correct file."
              );
          ...
          setError(
            "Error parsing XHTML file. Please ensure it's a valid XHTML/HTML file."
          );
        }
      };
  ...
  const drawBlock = (
    ...
    const startX =
      (block.start / duration) * canvasWidth * zoomLevelVal - scrollOffsetVal;
    const endX =
      (block.end / duration) * canvasWidth * zoomLevelVal - scrollOffsetVal;
  ...
  const drawWaveform = useCallback(() => {
    ...
    // Draw progress line
    const progressX =
      (currentTime / duration) * width * zoomLevel - scrollOffset;
  ...
  const getXFromTime = useCallback(
    (time) => {
      ...
      const currentCanvasWidth = waveformContainerRef.current
        ? waveformContainerRef.current.clientWidth
        : canvas.width;
      return (
        (time / duration) * currentCanvasWidth * zoomLevel -
  ...
  const getTimeFromX = useCallback(
    (x) => {
      ...
      return (
        ((x + scrollOffsetRef.current) / (currentCanvasWidth * zoomLevel)) *
        duration
      );
    },
    [duration, zoomLevel]
  ...
  const handleCanvasMouseDown = (e) => {
    if (enablePanning) {
      ...
      console.log(
        "scrollOffsetRef.current (at mousedown):",
        scrollOffsetRef.current
      );
  ...
  const handleCanvasMouseMove = (e) => {
    if (enablePanning) {
      ...
      const currentCanvasDisplayWidth = waveformContainerRef.current
        ? waveformContainerRef.current.clientWidth
        : canvas.width;
      const _totalContentWidth = currentCanvasDisplayWidth * zoomLevel;
      const _maxScrollOffset = Math.max(
        0,
        _totalContentWidth - currentCanvasDisplayWidth
      );

      if (isDraggingWaveform.current && dragStartX.current !== null) {
  ...
  const handleQuickReorder = () => {
    ...
    if (
      ...
      setError(
        `Please enter a valid position between 1 and ${editableBlocks.length}`
      );
      return;
    }
  ...
  const zoomIn = () => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const currentDisplayWidth = waveformContainerRef.current
      ? waveformContainerRef.current.clientWidth
      : canvas.width;
    ...
    const _scrollerNextOffsetInPixel =
      currentScrollRatio * _totalNextContentWidth;

    setScrollOffset((prev) => {
      // Ensure the new scroll offset doesn't go negative or past the end of the new content
      const maxScroll = Math.max(
        0,
        _totalNextContentWidth - currentDisplayWidth
      );
      const newOffset = Math.min(
        Math.max(0, _scrollerNextOffsetInPixel),
        maxScroll
      );
      scrollOffsetRef.current = newOffset; // Immediately update the ref
      return newOffset;
  ...
  const handleTimeUpdate = () => {
    if (mediaRef.current && canvasRef.current) {
      ...
      const timePixelPosition =
        (mediaRef.current.currentTime / duration) *
        currentCanvasDisplayWidth *
        zoomLevel;

      const targetScrollOffset =
        timePixelPosition - currentCanvasDisplayWidth / 2; // Attempt to center
      const maxScroll = Math.max(
        0,
        currentCanvasDisplayWidth * zoomLevel - currentCanvasDisplayWidth
      );
      ...
      if (
        timePixelPosition < canvasLeftEdge + margin ||
        timePixelPosition > canvasRightEdge - margin
      ) {
        setScrollOffset((prev) => {
          const newOffset = Math.min(
            Math.max(0, targetScrollOffset),
            maxScroll
          );
          scrollOffsetRef.current = newOffset; // Immediately update the ref
          return newOffset;
  ...
  const deleteBlock = (blockIndex) => {
    ...
    const updatedBlocks = editableBlocks.filter(
      (_, index) => index !== blockIndex
    );
  ...
  const resetChanges = () => {
    ...
    if (transcriptionData) {
      const blocks = getRawDisplayData(transcriptionData).map(
        (item, index) => ({
          id: `block-${index}`,
          text: item.text || item.word,
          start: item.start,
  ...
  const exportJSON = () => {
    ...
    // Extract all start points and end points from editable blocks
    const startpoints = editableBlocks.map((block) => block.start);
    const endpoints = editableBlocks.map((block) => block.end);
    ...
    link.href = url;
    link.download = `transcription_${viewMode}_${
      mediaFile?.name || "audio"
    }.json`;
    link.click();
  ...
  const exportAudacityLabels = () => {
    ...
    editableBlocks.forEach((block, index) => {
      labelContent += `${block.start.toFixed(6)}\t${block.end.toFixed(6)}\t${
        block.text
      }\n`;
    });
  ...
  const exportSMIL = () => {
    ...
    editableBlocks.forEach((block, index) => {
      ...
      const clipBegin = previousEndTime.toFixed(6) + "s";
      const clipEnd = block.end.toFixed(6) + "s";
  ...
  const exportSMIL_old = () => {
    ...
    editableBlocks.forEach((block, index) => {
      ...
      smilContent += `\t\t<par id="${parId}"><text src="../page-0001.xhtml#${smlId}"/><audio src="../audio/${
        mediaFile?.name || "audio.mp3"
  ...
  const handleCustomTextChange = useCallback((newText) => {
  ...
  }, []); // Empty dependency array is correct as it only uses React setters
            ...
            <div className="col-12">
              <div className="text-center mb-4">
                ...
                <p className="lead text-muted">
                  Upload, transcribe, and edit audio with precise timing
                  controls
                </p>
              </div>
                              ...
                              <div className="mb-3">
                                <div className="card">
                                  <div className="card-header bg-info text-white">
                                    ...
                                    <small>
                                      Edit the text and create custom break
                                      points
                                    </small>
                                  </div>
                                  <div className="card-body">
                                    <div className="mb-3">
                                      ...
                                      />
                                      <small className="text-muted">
                                        Upload an XHTML file to populate the
                                        text area.
                                      </small>
                                    </div>
                                    <div className="mb-3">
                              ...
                              <div className="card-body border-bottom bg-light p-3">
                                <div className="d-flex align-items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setQuickReorderPosition(e.target.value)
                                    }
                            ...
                            >
                              <div className="p-3">
                                {editableBlocks.map((block, index) => {
                                  ...
                                  const words =
                                    transcriptionData?.words?.filter(
                                      (word) =>
                                        word.start >= block.start &&
                                        word.end <= block.end
                                    ) || [];
